Shader "Hidden/MediaPipe/EmotionDetection/Visualizer"
{
    CGINCLUDE

    #include "UnityCG.cginc"
    #include "Packages/jp.off.mediapipe.emotiondetection/Shader/Struct.hlsl"

    StructuredBuffer<EmotionDetection> _Detections;

    float4 VertexBox(uint vid : SV_VertexID,
                     uint iid : SV_InstanceID) : SV_Position
    {
        EmotionDetection d = _Detections[iid];
